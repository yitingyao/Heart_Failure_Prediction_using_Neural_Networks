...
x = pd.get_dummies(x)
# This splits the dataset into two parts: one for training the model (70% of the data) and one for testing the model
# (30% of the data). The function train_test_split randomly divides the rows of x and y into training sets (X_train,
# Y_train) and testing sets (X_test, Y_test) while maintaining the correspondence between x (features) and y (labels).
...
# Convert integer labels into a binary matrix format required for categorical crossentropy loss. This is necessary
# because the model's output layer will be using softmax activation, which requires input in this format.
...
# Initialize the ConfusionMatrixDisplay object with the confusion matrix 'cm'. This object will allow you to display
# matrix.
cm = confusion_matrix(y_true, y_estimate)
# Display the confusion matrix using the display object. The 'cmap=plt.cm.Blues' parameter colors the matrix in
# shades of blue.
